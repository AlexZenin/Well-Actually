	...
	function getResults(){
			...
			$.post('/post', { url: url }, function(response) {
				...
				for (index in results) {
					...
					$('#results').append('<div class="column"><div class="ui fluid card"><div class="image"><img src="' + results[index].imageURL + '"></div><div class="content"><a href="' + results[index].title + '" class="header">' + results[index].title + '</a><div class="description">' + results[index].content + '</div></div><div class="extra content"><span class="right floated">' + sentimentTag + '</span></div></div></div>')
				}
			})
	}
})